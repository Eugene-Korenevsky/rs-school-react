...
export default function Form(formProps: FormProps): JSX.Element {
    ...
    const onCheckBoxHandler = (event: React.FormEvent): void => {
        const res = (event.target as HTMLInputElement).checked ? true : false;
        setAgree(res);
    }

    const getInputValue = (event: React.FormEvent): string => {
    ...
    return <form onSubmit={onFormSubmit} className="form">
        ...
        <InputAgree onChangeHandler={(event: React.FormEvent): void => { onCheckBoxHandler(event) }} errors={errors}></InputAgree>
